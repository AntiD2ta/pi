...
Use `ctx.ui.setHeaderWidget(key, factory)` to add keyed content below Pi's startup header without replacing it; pass `undefined` as the factory to remove that key.
