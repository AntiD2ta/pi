...
Prompt history is stored in `~/.pi/agent/prompt-history.json`. Entries are deduplicated by trimmed exact text, reused prompts move to the newest position, and only the newest 100 are shown. Prompts larger than 16 KiB, non-interactive input, and `--no-session` runs are not persisted.
